GUM <- function(data, C, method = "MML", 
                N.nodes = 30, max.outer = 60, max.inner = 60, tol = .001, 
                precision = 4, verbose = "intermediate")
{
    data.condensed <- GGUM.data.condense(data)$data.condensed
    I              <- ncol(data.condensed) - 1
    S              <- nrow(data.condensed)
    # Because models 4 and 8 (based on possibly different C per item) need function 
    # GUM() to estimate starting values, the code of GUM() needs to be versatile 
    # and allow for a vector of varying C values.
