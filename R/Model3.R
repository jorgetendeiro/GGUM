Model3 <- function(data, C, method = "MML", 
                   N.nodes = 30, max.outer = 60, max.inner = 60, tol = .001, verbose = "intermediate")
{
  data.condensed <- GGUM.data.condense(data)$data.condensed
  I              <- ncol(data.condensed) - 1
  S              <- nrow(data.condensed)
  # Because models 4 and 8 (based on possibly different C per item) need function Model3() to estimate starting values, 
  #    the code of Model3() needs to be versatile and allow for a vector of varying C values.
  # Observe that Model 3 assumes C fixed for all items.
  # Below, we therefore accommodate the code for vector of C values.
  if (length(C) == 1) {C <- rep(C, I)}
  C.max          <- max(C)
  M              <- 2 * C + 1
  
  # Initial values derived using the procedure in the appendix of Roberts and Laughlin (1996):
  ...
  while ((iter.outer <= (max.outer - 1)) && (iter.inner > 1))
  {
    ...
    #             given the current delta, and taus.
    # Ls:
...
#       Ls(data.condensed[, c(it.use, I + 1), drop = FALSE], rep(1, C.tab.frqs[c]), delta.old[it.use], 
#          taus.old[it.use, , drop = FALSE][, (C.max + 1 - c.use):(C.max + 1 + c.use)], nodes, c.use)
#     })
...
#     Ls.mat            <- apply(Ls.mat, 1:2, prod)                                                 # S x N.nodes
    ...
    while ((iter.inner <= (max.inner - 1)) && (max(curr.tol) > tol))
    {
      ...
      DlogL.taus     <- colSums(D1$taus) # C.max
      # 
      ...
      # Inf.arr <- apply((N*I) * dP.taus.taus / P.izf.arr.taus.taus, c(4, 5), sum)
      # 
  ...
  return(list(alpha = alpha, delta = delta.old, taus = taus.old, 
              N.nodes = N.nodes, tol = max(curr.tol), iter.inner = iter.inner, model = "GUM", InformationCrit = Inf.df))
}
